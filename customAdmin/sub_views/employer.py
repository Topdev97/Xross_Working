...
def index(request):
    try:            
        ...
        m_list = User.objects.filter(username__contains=keyword, user_type = 'employer', is_staff=0).order_by('username')
        ...
        return render(request, "admin/employers/index.html", {"page_obj": page_obj, "search_message": search_message ,"keyword": keyword, "page_number": page_number, "total_count": m_list.count()})
    except:
        messages.warning(request, "リクエストのパラメータが正しくありません。")

    return render(request, "admin/employers/index.html")
...
def update(request):
    ...
    return render(request, "admin/employers/update.html", {"form": form, "current_user": m_user})
